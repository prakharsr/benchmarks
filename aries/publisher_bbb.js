...
async function createConnections(numberOfConnections) {
    try {
        ...
        while(globalConnections.length < numberOfConnections) {
            ...
            var command = `aca-py start --inbound-transport http 192.168.7.2 ${(globalConnections.length+1) + 8010} --outbound-transport http --genesis-url http://192.168.7.1:9000/genesis --wallet-storage-type postgres_storage --wallet-storage-config {"url":"http://192.168.7.1:5432"} --admin 0.0.0.0 ${(globalConnections.length+1) + 8012} --admin-insecure-mode -e http://192.168.7.2:${(globalConnections.length+1) + 8010}`;
